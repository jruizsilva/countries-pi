...
const getAllCountries = async (req = request, res = response) => {
  ...
  if (success) {
    ...
    const { results } = await getCountriesDB(page, continentName, sort);
    if (results && results.length > 0) {
      res.json(results);
    }
  } else {
...
const getCountryById = async (req = request, res = response, next) => {
  const { id } = req.params;
  try {
...
const getCountriesByName = async (req = request, res = response, next) => {
...
*/
const getActivitiesByCountry = async (req = request, res = response, next) => {
  const { id } = req.params;
  try {
